...
```bash
pip install numpy pandas scipy pywavelets scikit-learn xgboost lightgbm catboost optuna joblib
```
...
| `--trials` | `50` | Количество Optuna-итераций на модель (80–120 для финала) |
| `--features` | `120` | Сколько признаков оставить после отбора |
| `--models` | все | Выбор базовых моделей из: `xgb lgb cat rf et svm mlp` |

Обученная модель сохраняется в один `.pkl` файл — включает все трансформеры, признаки и мета-learner.
